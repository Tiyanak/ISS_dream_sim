...
public class RewardFlashing : MonoBehaviour {
    ...
    public int rewardTrial = 20;
    public int nonRewardTrial = 80;
    public int totalTrials = 100; // reward + nonReward
    ...
    public void Flash()
    {
        if (!isFlashing)
        {
          ...
          if (lastSquare.Equals("RewardSquare"))
            {
                showSquare = mapSquares.GetNonRewardSquare();
                lastSquare = showSquare.name;
                nonRewardTrial -= 1;
            }
            else
            {
                showSquare = mapSquares.PickRandomRewardSquare((double)rewardTrial / nonRewardTrial);
                lastSquare = showSquare.name;
                if (lastSquare.Equals("RewardSquare"))
                {
                    rewardTrial -= 1;
        ...
        else
        {
            ...
            else if (waitUntilClickSquareTime + showSquareShowTime < currentTrialTime && currentTrialTime < waitUntilClickSquareTime + showSquareShowTime + clickSquareShowTime)
            {
                ...
                if (Input.GetKeyDown("space") && !successfullClick)
                {
                    ...
                    if (showSquare.name.Equals("RewardSquare")) {
                        playerScore += 1;
                        scoreText.text = playerScore.ToString() + "$/" + currentTrialNumber.ToString();
                    }
                }
